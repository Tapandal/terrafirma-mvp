...
def _mask_s2(img: ee.Image, cfg: Config) -> ee.Image:
    ...
    # ee.Image(...) around copyProperties is required, not cosmetic: copyProperties
    # returns an ee.Element, and Elements have no addBands/normalizedDifference. Without
    # the cast the next step in the chain fails with
    # "'Element' object has no attribute 'addBands'".
    return ee.Image(
        img.updateMask(clear).divide(10000).copyProperties(img, ["system:time_start"])
    )
...
def underwrite_location(lat: float, lon: float, cfg: Config = DEFAULT_CONFIG) -> Dict:
    ...
    Both paths are wrapped: the embedding route can fail for reasons that have nothing to
    do with the fallback route (no coverage for a year, a transient Earth Engine error),
    and in that case the Sentinel-only score is still worth having.
    """
    ...
    try:
        ...
        if result.get("status") == "ok":
            return result
        embedding_error = result.get("message") or result.get("status")
    except Exception as exc:  # noqa: BLE001
        embedding_error = str(exc)

    try:
        fallback = stability_score_sar_optical(lat, lon, cfg)
        if embedding_error:
            fallback["embedding_unavailable"] = embedding_error
        return fallback
    except Exception as exc:  # noqa: BLE001
        return {
            "status": "error",
            "message": f"Both scoring routes failed. Embeddings: {embedding_error}. "
                       f"Sentinel fallback: {exc}",
        }
...
def assess_event(
    ...
    out: Dict = {"lat": lat, "lon": lon, "event_date": event_date}

    # Each of the four analyses is independent. Cloud cover can kill the optical check
    # while the radar check is perfectly good, and the radar check is the one that drives
    # the payout — so a failure in any one must not take the others down with it.
    def _attempt(label: str, fn) -> None:
        try:
            out[label] = fn()
        except Exception as exc:  # noqa: BLE001
            out[label] = {"status": "unavailable", "reason": str(exc)}

    _attempt("rapid_sar", lambda: rapid_sar_anomaly(lat, lon, event_date, cfg))
    _attempt("rapid_optical", lambda: rapid_optical_anomaly(lat, lon, event_date, cfg))
    _attempt("annual_embedding", lambda: event_anomaly(lat, lon, event_year, cfg))
    _attempt("pre_loss_context", lambda: stability_score(lat, lon, cfg))
