...
class T:
    ...
    ink = "#16202B"        # primary text, deep blue-slate
    graphite = "#5C6B77"   # secondary text and rules
    mist = "#AEBAC2"       # tertiary / disabled
    field = "#F2F4F3"      # page ground, cool survey-margin grey
    panel = "#FFFFFF"      # card surface
    rule = "#DFE5E4"       # hairlines

    signal = "#0B5D6B"     # accent, petrol teal — instrument dial
    signal_soft = "#E3EDEE"

    clear = "#1F7A4C"      # status: normal
    watch = "#A8631B"      # status: elevated, below trigger
    triggered = "#B4271F"  # status: payout triggered
...
def render_underwriter(a: Assessment) -> None:
    ...
    with left:
        st.markdown(
            ...
            f'<span class="pv-eyebrow">Pre-loss vulnerability</span>'
    ...
    with right:
        st.markdown(
            ...
            f'<span class="pv-eyebrow">Annual passes · deviation from site norm</span>'
    ...
    m1.metric("Pricing tier", tier.label)
    m2.metric("Property rate", f"{tier.base_rate_pct:.2f}%")
    ...
    with c1:
        bars = "".join(
            ...
            f"</div>"
            for key, label in (
                ("churn", "Year-on-year churn"),
                ("tail", "Worst single year"),
                ("dispersion", "Spread about norm"),
                ("regime", "Directional drift"),
            )
        )
        weights = (
            f"Weighted {POLICY.w_churn:.2f} / {POLICY.w_tail:.2f} / "
            f"{POLICY.w_dispersion:.2f} / {POLICY.w_regime:.2f}."
        )
        st.markdown(
            f'<div class="pv-panel"><span class="pv-eyebrow">Score components</span>{bars}'
            f'<p class="pv-note">{weights} Directional drift carries the same weight as '
            f"churn because a site that is converting will not revert, while a site that "
            f"merely oscillates will.</p></div>",
            unsafe_allow_html=True,
        )
    ...
    with c2:
        rows = [
            ...
            (f"Parametric layer @ {tier.parametric_rate_pct:.2f}%", money(a.parametric_premium), True),
    ...
    st.markdown(
        f'<div class="pv-panel"><span class="pv-eyebrow">Parametric status</span>'
        f'<div style="display:flex;align-items:center;gap:1.4rem;flex-wrap:wrap;">'
        f"{badge_html(a.status)}"
        f'<span class="pv-fig pv-fig--sm">Disturbed area {a.damage_fraction * 100:.1f}% · '
        f"Trigger at {POLICY.payout_tiers[0][0] * 100:.0f}% · "
        f"Layer limit {money(a.parametric_limit)}</span></div>"
        f'<p class="pv-note">{_status_note_underwriter(a)}</p></div>',
        unsafe_allow_html=True,
    )

    with st.expander("Raw metrics and audit trail"):
        st.markdown(
...
def _status_note_underwriter(a: Assessment) -> str:
    if a.status == "triggered":
        return (
            f"Radar detected disturbance across {a.damage_fraction * 100:.1f}% of the "
            f"insured radius, past the {POLICY.payout_tiers[0][0] * 100:.0f}% trigger. "
            f"Payout of {money(a.payout_amount)} "
            f"({a.payout_fraction * 100:.0f}% of layer limit) is due without loss adjustment."
        )
    if a.status == "watch":
        return (
            "Some disturbance detected, below the contractual trigger. No payout is due. "
            "Flag for the next portfolio review if it persists across two more passes."
        )
    return "No disturbance beyond this site's normal year-to-year variation. No payout due."
...
def render_policyholder(a: Assessment) -> None:
    ...
    m2.metric("Automatic payout limit", money(a.parametric_limit))
    m3.metric(
    ...
    with c1:
        ...
        st.markdown(
            f'<div class="pv-panel"><span class="pv-eyebrow">Your location, year by year</span>'
            f"{pass_strip_svg(a.years, a.year_distances, a.status != 'clear', a.status)}"
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        ee_project, radius = None, 500.0
        if source == "Live satellite":
            ee_project = st.text_input("Cloud project", placeholder="my-ee-project")
        ...
        st.markdown("---")
        st.markdown(
            '<p class="pv-note" style="font-size:.78rem;">Scores come from AlphaEarth '
            "annual satellite embeddings; the parametric trigger runs on Sentinel-1 radar, "
            "which sees through cloud.</p>",
            unsafe_allow_html=True,
        )
    ...
    with st.form("location", border=False):
        ...
        lat = c2.number_input("Latitude", value=22.3039, format="%.5f", min_value=-90.0, max_value=90.0)
        lon = c3.number_input("Longitude", value=70.8022, format="%.5f", min_value=-180.0, max_value=180.0)
        ...
        try:
            with st.spinner("Reading satellite history for this location…"):
                if source == "Live satellite":
                    if not ee_project:
                        st.error(
                            "Live mode needs a Google Cloud project ID. Add one in the sidebar, "
                            "or switch to demo data."
                        )
                        return
                    geo_risk.initialize(ee_project)
