...
def live_assessment(
    ...
    if sar.get("status") == "ok":
        ...
        notes.append(f"Radar check unavailable: {sar.get('reason') or sar.get('status')}")

    optical = event.get("rapid_optical", {})
    if optical and optical.get("status") != "ok":
        notes.append(
            "Optical check unavailable (usually cloud cover). The radar check is "
            "cloud-proof and is what drives the payout, so this is rarely a problem."
        )
    ...
    if annual.get("status") == "ok":
        ...
        notes.append(
            "Yearly satellite comparison unavailable — normal for a recent event, "
            "since those maps are only published once a year."
        )
    ...
    if stab.get("method") != "alphaearth_embedding":
        notes.insert(0, "Yearly satellite maps unavailable for this spot — the score was "
                        "built from Sentinel-1 and Sentinel-2 directly instead.")
    if stab.get("embedding_unavailable"):
        notes.append(f"Reason: {stab['embedding_unavailable']}")
...
def _ee_session() -> str:
    """Open the Earth Engine connection. Returns the project ID.

    This raises on failure rather than returning a flag, and that is deliberate:
    st.cache_resource does not cache a call that raised. If it returned False instead,
    a failed connection would be cached and the app would keep showing the same stale
    error even after you fixed the permissions in Google Cloud.
    """
    if not GEO_RISK_AVAILABLE:
        raise RuntimeError(
            "geo_risk.py is not in the repository, or earthengine-api is not installed."
        )
    ...
    if missing:
        raise RuntimeError(f"Secrets are incomplete — missing: {', '.join(missing)}.")

    creds = ee.ServiceAccountCredentials(info["client_email"], key_data=json.dumps(info))
    ee.Initialize(creds, project=info["project_id"])
    ee.Number(1).getInfo()  # cheap round trip to prove the connection really works
    return str(info["project_id"])


def _explain_ee_error(raw: str, account: str, project: str) -> Tuple[str, List[str]]:
    """Turn a raw Google error into a plain-language cause and a numbered fix."""
    low = raw.lower()

    if "serviceusage" in low or "does not have required permission" in low:
        return (
            "The robot account exists, but it is not allowed to use this project yet. "
            "It needs one more role: Service Usage Consumer.",
            [
                f"Open console.cloud.google.com/iam-admin/iam?project={project}",
                "Click **Grant access** (or **+ ADD**) at the top",
                f"In **New principals**, paste: {account or 'your service account email'}",
                "Add role **Service Usage Consumer**",
                "Click **+ Add another role**, add **Earth Engine Resource Viewer**",
                "Click **Save**, wait 2–5 minutes, then press Retry connection below",
            ],
        )
    if "not registered" in low or "not been used" in low or "is disabled" in low:
        return (
            "The Earth Engine API is not switched on for this project, or the robot "
            "account has not been registered with Earth Engine.",
            [
                f"Open console.cloud.google.com/apis/library/earthengine.googleapis.com?project={project}",
                "Click **Enable** (if it says Manage, it is already on)",
                "Go to code.earthengine.google.com → gear/settings icon",
                f"Register this service account: {account or 'your service account email'}",
                "Wait 2 minutes, then press Retry connection below",
            ],
        )
    if "permission" in low and "earthengine" in low:
        return (
            "The robot account is missing its Earth Engine role.",
            [
                f"Open console.cloud.google.com/iam-admin/iam?project={project}",
                f"Find {account or 'your service account'} and click the pencil to edit it",
                "Add role **Earth Engine Resource Viewer**",
                "Save, wait 2 minutes, then press Retry connection below",
            ],
        )
    if "padding" in low or "private_key" in low or "decode" in low or "pem" in low:
        return (
            "The private key in your Secrets box is damaged — usually from line breaks "
            "being added when it was pasted.",
            [
                "Go to share.streamlit.io → your app → Settings → Secrets",
                "Re-paste private_key so it sits entirely on ONE line",
                "Keep every \\n exactly as it appears in the JSON file",
                "Make sure the whole value is wrapped in double quotes",
                "Save, then press Retry connection below",
            ],
        )
    return (
        "Earth Engine refused the connection. The exact message from Google is below.",
        ["Check the setup steps again, then press Retry connection below"],
    )


def init_earth_engine() -> Tuple[bool, str]:
    """Connect to Earth Engine. Returns (connected, project id or raw error)."""
    try:
        return True, _ee_session()
...
def inject_css() -> None:
    st.markdown(
        ...
        button[data-testid="stBaseButton-primaryFormSubmit"] {{
            background: {T.signal} !important; color: #fff !important; border: none;
            border-radius: 2px;
        ...
        .stButton > button:hover, .stFormSubmitButton > button:hover {{
            background: #063E47 !important; color: #fff !important;
        }}
        .stButton > button:focus-visible, .stFormSubmitButton > button:focus-visible {{
            outline: 3px solid {T.watch}; outline-offset: 2px;
        }}

        /* Accent colours for radios, checkboxes and sliders. These normally come from
           primaryColor in .streamlit/config.toml, but that file is easy to miss, and
           without it Streamlit falls back to its default red — which clashes badly and
           makes the status colours ambiguous. Forcing them here means the app looks
           right whether or not config.toml made it into the repository. */
        .stRadio [data-baseweb="radio"] div[aria-checked="true"],
        .stRadio [role="radio"][aria-checked="true"] {{
            background-color: {T.signal} !important; border-color: {T.signal} !important;
        }}
        .stCheckbox [data-baseweb="checkbox"] span[aria-checked="true"],
        .stCheckbox span[data-baseweb="checkbox"] > span[aria-checked="true"] {{
            background-color: {T.signal} !important; border-color: {T.signal} !important;
        }}
        [data-baseweb="slider"] [role="slider"] {{
            background-color: {T.signal} !important; border-color: {T.signal} !important;
        }}
        [data-baseweb="slider"] div[data-testid="stSliderTickBarMin"] ~ div > div,
        [data-testid="stSlider"] [data-baseweb="slider"] > div > div > div:first-child {{
            background: {T.signal} !important;
        }}
        [data-testid="stSliderThumbValue"] {{ color: {T.signal} !important; }}
...
def render_underwriter(a: Assessment) -> None:
    ...
    if a.notes:
        st.markdown("<div style='height:.9rem'></div>", unsafe_allow_html=True)
        items = "".join(f"<li style='margin-bottom:.3rem;'>{n}</li>" for n in a.notes)
        st.markdown(
            f'<div class="pv-panel" style="border-left:3px solid {T.watch};">'
            f'<span class="pv-eyebrow">Data availability</span>'
            f'<ul class="pv-note" style="margin:0;padding-left:1.1rem;">{items}</ul>'
            f'<p class="pv-note" style="margin-top:.6rem;">The score and payout above use '
            f"whatever checks did run. Missing checks make the result less certain, not "
            f"wrong.</p></div>",
            unsafe_allow_html=True,
        )
    ...
    with st.sidebar:
        ...
        if source == "Live satellite":
            ...
            if ee_ready:
                ...
                secrets_block = st.secrets.get("gcp_service_account", {})
                account = str(secrets_block.get("client_email", ""))
                project = str(secrets_block.get("project_id", "your-project"))
                cause, fix_steps = _explain_ee_error(ee_detail, account, project)
                ...
                if st.button("Retry connection"):
                    _ee_session.clear()
                    st.rerun()
                with st.expander("Exact message from Google"):
                    st.code(ee_detail, language=None)
